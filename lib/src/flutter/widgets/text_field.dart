...
class TextField extends Relement {
  ...
  final DivElement _wrap =
      DivElement(); // wrapper champ (bordure appliquée ici)
  ...
  Element create() {
    ...
    _input
      ..className = 'rdx-input'
      ..placeholder =
          ' ' // pour label flottant via :placeholder-shown
    ...
    if (decoration.errorText != null && decoration.errorText!.isNotEmpty ||
        (_externalError != null && _externalError!.isNotEmpty)) {
  ...
  void _renderEye() {
    _eyeBtn.setInnerHtml(
      _obscuredRuntime
          ? _biconToString(Bicon.eye)
          : _biconToString(Bicon.eyeSlash),
      treeSanitizer: NodeTreeSanitizer.trusted,
    );
  }
  ...
  String _effectiveHtmlType() {
    ...
    switch (inputType) {
      ...
      case InputType.number:
        return 'text'; // inputmode numeric/decimal pour meilleur contrôle
  ...
  // --- NEW: API publique pour piloter l'erreur dynamiquement ---
  void setErrorText(String? msg) {
    _externalError = (msg != null && msg.isNotEmpty) ? msg : null;
    final has = _externalError != null;
    _root.classes.toggle('is-error', has);
    _helper.classes.toggle('is-error', has);
    _helper.text = has ? _externalError! : (decoration.helperText ?? '');
    _applyBorder();
  }

  InputBorder _borderFromVariant({
    bool focused = false,
    bool error = false,
    bool disabled = false,
  }) {
